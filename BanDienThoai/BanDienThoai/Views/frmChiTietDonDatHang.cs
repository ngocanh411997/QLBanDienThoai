...
namespace BanDienThoai.Views
{
    public partial class frmChiTietDonDatHang : Form
    {
        ...
        {
            DataTable dt = new DataTable();
            dt = Bus.GetListSP("SELECT * FROM dbo.tbl_sanpham WHERE ten NOT IN (SELECT ten FROM dbo.tbl_chitietdonhang INNER JOIN dbo.tbl_sanpham ON tbl_sanpham.id = tbl_chitietdonhang.id_sanpham WHERE id_dondathang LIKE '"+txtMaDon.Text+"') AND TrangThai=0");
            cbSanPham.DataSource = dt;
            cbSanPham.DisplayMember = "ten";
            cbSanPham.ValueMember = "id";
        }
